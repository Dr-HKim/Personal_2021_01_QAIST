...
df_etf = pd.merge(df_etf, krx_etf_market_price[["종목코드", "시가총액", "거래량", "거래대금", "종가", "순자산가치(NAV)"]], left_on='종목번호', right_on='종목코드', how='left')
df_etf.pop("단축코드")
df_etf.pop("종목코드")
df_etf["시총대비거래대금"] = df_etf["거래대금"]/df_etf["시가총액"]
df_etf["시가총액"] = (df_etf["시가총액"] / 100000000).round(2)
df_etf["거래대금"] = (df_etf["거래대금"] / 100000000).round(2)


df_etf2 = df_etf[["구분", "종목번호", "한글종목약명", "기초지수명", "총보수", "과세유형", "시가총액", "종가", "상장일", "거래량", "거래대금", "순자산가치(NAV)"]]

# 엑셀 저장
df_etf.to_excel('./KRX_raw/list_ETF2.xlsx', index=False, index_label=None)
