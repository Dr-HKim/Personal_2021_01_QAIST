...
dg_fs_IFRSC_2000_2009_data = pd.read_pickle('./DataGuide_processed/dg_fs_IFRSC_2000_2009_data.pkl')
dg_fs_IFRSC_2010_2019_data = pd.read_pickle('./DataGuide_processed/dg_fs_IFRSC_2010_2019_data.pkl')
dg_fs_IFRSC_2020_2021_data = pd.read_pickle('./DataGuide_processed/dg_fs_IFRSC_2020_2021_data.pkl')

dg_fs_IFRSC_2000_2009_header = pd.read_pickle('./DataGuide_processed/dg_fs_IFRSC_2000_2009_header.pkl')
dg_fs_IFRSC_2010_2019_header = pd.read_pickle('./DataGuide_processed/dg_fs_IFRSC_2010_2019_header.pkl')
dg_fs_IFRSC_2020_2021_header = pd.read_pickle('./DataGuide_processed/dg_fs_IFRSC_2020_2021_header.pkl')
...
fs_IFRSC = pd.merge(
    fs_IFRSC, dg_GeneralHistoric_Quarterly[["Symbol", "회계년", "주기", "결산월(Hist)", "거래소(시장)"]],
...
# 기업x회계년 마다 더미변수를 연결
fs_IFRSC = pd.merge(
    fs_IFRSC, df_index3[["Symbol", "회계년", "check_nan_123Q"]],
    left_on=["Symbol", "회계년"], right_on=["Symbol", "회계년"], how='left')

########################################################################################################################
# 기업코드x회계연도 별로 결산월 값이 하나인(unique) 기업코드x회계연도 체크
check_unique_settle_month = fs_IFRSC.groupby(["Symbol", "회계년"])["결산월"].nunique().eq(1)
fs_IFRSC = pd.merge(fs_IFRSC, check_unique_settle_month, left_on=["Symbol", "회계년"], right_index=True, how='left')
fs_IFRSC.rename(columns={"결산월_x":"결산월", "결산월_y":"check_unique_settle_month"}, inplace=True)

# 기업코드 별로 결산월(Hist) 값이 모두 비어있는 기업코드 체크
check_null_settle_month_hist = fs_IFRSC.groupby("Symbol")['결산월(Hist)'].apply(lambda x: x.isnull().all())
fs_IFRSC = pd.merge(fs_IFRSC, check_null_settle_month_hist, left_on=["Symbol"], right_index=True, how='left')
fs_IFRSC.rename(columns={"결산월(Hist)_x":"결산월(Hist)", "결산월(Hist)_y":"check_null_settle_month_hist"}, inplace=True)

# 기업코드 별로 결산월(Hist) 값이 하나인(unique) 기업코드 체크
check_unique_settle_month_hist = fs_IFRSC.groupby(["Symbol"])["결산월(Hist)"].nunique().eq(1)
fs_IFRSC = pd.merge(fs_IFRSC, check_unique_settle_month_hist, left_on=["Symbol"], right_index=True, how='left')
fs_IFRSC.rename(columns={"결산월(Hist)_x":"결산월(Hist)", "결산월(Hist)_y":"check_unique_settle_month_hist"}, inplace=True)

########################################################################################################################
# 제출날짜 구하기
fs_IFRSC["제출월2"] = fs_IFRSC["제출월"] + 1
fs_IFRSC["제출년2"] = fs_IFRSC["제출년"]
fs_IFRSC.loc[fs_IFRSC["제출월2"] > 12, "제출년2"] = fs_IFRSC["제출년"] + 1
fs_IFRSC.loc[fs_IFRSC["제출월2"] > 12, "제출월2"] = fs_IFRSC["제출월2"] - 12
fs_IFRSC["제출날짜"] = fs_IFRSC["제출년2"]*10000 + fs_IFRSC["제출월2"]*100 + 1
fs_IFRSC["제출날짜"] = pd.to_datetime(fs_IFRSC["제출날짜"].astype(str), errors='coerce', format='%Y%m%d')
fs_IFRSC["제출날짜"] = fs_IFRSC["제출날짜"] - pd.Timedelta(days=1)
fs_IFRSC["사용가능날짜"] = fs_IFRSC["제출날짜"] + pd.Timedelta(days=1)
fs_IFRSC.pop("제출월2")
fs_IFRSC.pop("제출년2")

########################################################################################################################
# 컬럼 순서 바꾸기
column_names = fs_IFRSC.columns.tolist()
...
fs_IFRSC = fs_IFRSC[new_column_names]

########################################################################################################################
# 데이터 저장하기
fs_IFRSC.to_pickle('./DataGuide_processed/fs_IFRSC.pkl')

# 재무제표 자료는 2011년 이후부터 분기별 자료 사용 가능
fs_IFRSC = pd.read_pickle('./DataGuide_processed/fs_IFRSC.pkl')
fs_IFRSC_sample = fs_IFRSC.loc[fs_IFRSC["회계년"] > 2015]

fs_IFRSC_sample.to_pickle('./DataGuide_processed/fs_IFRSC_sample.pkl')
